...
確認已將助教 GitHub 帳號加為 Collaborator（Settings $\to$ Collaborators）。
...
\begin{figure}[H]
  ...
  \caption{Easy Social 登入頁面（含「李亮節's Easy Social」品牌名稱）}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{screenshots/04_feed_after_register.png}
  \caption{註冊成功後的 Feed 頁面}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{screenshots/05_feed_with_post.png}
  \caption{發文功能正常運作}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{screenshots/10_profile_follow.png}
  \caption{追蹤功能正常運作}
\end{figure}
...
\begin{tcolorbox}[colback=lightblue, colframe=accent, title=部署連結]
\href{https://easy-social-zeta.vercel.app}{\texttt{https://easy-social-zeta.vercel.app}}
\end{tcolorbox}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{screenshots/02_register_captcha.png}
  \caption{註冊頁面顯示 CAPTCHA 驗證問題}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{screenshots/03_captcha_wrong.png}
  \caption{輸入錯誤答案時顯示錯誤訊息，帳號未被建立}
\end{figure}
...
\begin{tcolorbox}[colback=lightgreen, colframe=green!60!black, title=功能展示影片（YouTube）]
\href{https://youtu.be/Aro0cQ61iEg}{\texttt{https://youtu.be/Aro0cQ61iEg}}
\end{tcolorbox}
...
使用者可建立包含 2--4 個選項的投票貼文。投票前顯示單選按鈕供使用者選擇；投票後即時顯示各選項得票百分比，並以 \checkmark{} 標示使用者自己的選擇。每位使用者每則投票只能投票一次，由資料庫層 UNIQUE constraint 強制執行。
...
\midrule
\texttt{post}（修改） & \texttt{is\_poll} & 標記此貼文為投票貼文 & BOOLEAN NOT NULL DEFAULT FALSE \\
\midrule
\multirow{2}{*}{\texttt{poll}} & \texttt{id} & 主鍵 & INTEGER PK \\
 & \texttt{post\_id} & 關聯貼文（唯一） & INTEGER UNIQUE FK $\to$ post.id \\
\midrule
\multirow{4}{*}{\texttt{poll\_option}} & \texttt{id} & 主鍵 & INTEGER PK \\
 & \texttt{poll\_id} & 所屬投票 & INTEGER FK $\to$ poll.id \\
...
\multirow{5}{*}{\texttt{poll\_vote}} & \texttt{id} & 主鍵 & INTEGER PK \\
 & \texttt{poll\_id} & 所屬投票 & INTEGER FK $\to$ poll.id \\
 & \texttt{option\_id} & 投票選項 & INTEGER FK $\to$ poll\_option.id \\
 & \texttt{voter\_id} & 投票者 & INTEGER FK $\to$ user.id \\
...
\textbf{關聯性說明：}
\begin{itemize}[leftmargin=2em]
  \item \texttt{post} $\to$ \texttt{poll}：一對一（\texttt{post\_id} UNIQUE）
  \item \texttt{poll} $\to$ \texttt{poll\_option}：一對多（2--4 個選項）
  \item \texttt{poll} + \texttt{user} $\to$ \texttt{poll\_vote}：多對多，UNIQUE(\texttt{poll\_id}, \texttt{voter\_id}) 強制每人只能投票一次
\end{itemize}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{screenshots/06_poll_created.png}
  \caption{建立投票貼文——填寫問題與選項後顯示於 Feed}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.8\textwidth]{screenshots/07_poll_result.png}
  \caption{投票後即時顯示各選項百分比與已選標記}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.8\textwidth]{screenshots/08_poll_two_votes.png}
  \caption{多位使用者投票後的結果分佈}
\end{figure}
...
\midrule
E2E Test (Selenium) & 3 & Feed 頁顯示投票表單、建立投票顯示於 Feed、完整投票流程（建立$\to$投票$\to$顯示百分比）\\
\bottomrule
\end{tabularx}
...
\begin{tcolorbox}[colback=lightgreen, colframe=green!60!black, title=功能展示影片（YouTube）]
\href{https://youtu.be/Rm37oRIhw48}{\texttt{https://youtu.be/Rm37oRIhw48}}
\end{tcolorbox}
...
\begin{enumerate}[leftmargin=2em]
  \item \textbf{活動邀請取代破冰訊息} $\to$ 消除「不知道說什麼」的心理壓力
  \item \textbf{遊戲結果自動帶入對話} $\to$ 每段關係都有具體共同經歷作為開始
  \item \textbf{行為相容性數據} $\to$ 補足照片＋簡介的片面資訊，更準確預測對話品質
  \item \textbf{約會引導機制} $\to$ 在對話熱度高峰時主動推動線下見面，減少 Ghosting
\end{enumerate}
...
\begin{tikzpicture}[font=\small\ttfamily]
  ...
  \node at (6,11.5) {[Filter] [Settings]};
  \draw (1,1.5) rectangle (7,10.5);
  \fill[gray!30] (1.3,5.5) rectangle (6.7,10.2);
  \node at (4,7.8) {[Profile Photo]};
  \node[anchor=west] at (1.3,5.1) {Pinxuan, 28};
  \node[anchor=west] at (1.3,4.6) {[loc] Xinyi Dist. 3km};
  \node[anchor=west] at (1.3,4.1) {[tag] Board Games / Film / Hiking};
  \fill[accent!80] (1.8,2.8) rectangle (6.2,3.6);
  \node[white] at (4,3.2) {[game] Play Board Game Quiz};
  \node at (2.5,2.1) {[Skip]};
  \node at (5.5,2.1) {[Super Match]};
  \node at (4,1.2) {● ○ ○};
\end{tikzpicture}
\caption{畫面一：配對瀏覽頁——以活動邀請取代傳統刷卡}
\end{figure}

\textbf{互動說明}：點擊「Play Board Game Quiz」$\to$ 進入畫面二（問答遊戲）
...
\begin{tikzpicture}[font=\small\ttfamily]
  \draw[thick] (0,0) rectangle (8,12);
  \fill[gray!20] (0,11) rectangle (8,12);
  \node at (0.8,11.5) {[Back]};
  \node at (4,11.5) {Board Game Quiz};
  \node at (7,11.5) {2/5};
  ...
  \node at (5.9,5.7) {羊毛};
  \node at (4,4.5) {[Timer] 12s remaining};
...
\textbf{互動說明}：點選答案後自動進入下一題 $\to$ 5 題結束後進入畫面三
...
\begin{tikzpicture}[font=\small\ttfamily]
  ...
  \fill[gray!20] (0,11) rectangle (8,12);
  \node at (0.8,11.5) {[Back]};
  \node at (4,11.5) {It's a Match!};
  \fill[gray!30] (1,8.8) rectangle (3,10.5);
  \node at (2,9.65) {Pinxuan};
  \node at (4,9.65) {$<$3};
  \fill[gray!30] (5,8.8) rectangle (7,10.5);
  \node at (6,9.65) {You};
  \node at (4,8.3) {Both got 3 / 5 correct};
  \draw[gray!40] (1,7.9) -- (7,7.9);
  \node[anchor=west] at (1,7.4) {[v] Q1 --- Both correct};
  \node[anchor=west] at (1,6.9) {[x] Q3 --- Both wrong :D};
  \node[anchor=west] at (1,6.4) {[v] Q5 --- Both correct};
  \fill[lightyellow] (0.5,4.3) rectangle (7.5,6);
  \node[anchor=north west, text width=6.5cm] at (0.7,5.9) {Suggested opener:\\``We both missed Q3---do you\\play Catan often?''};
  \draw (0.5,3.3) rectangle (3.5,4);
  \node at (2,3.65) {Use this};
  \draw (4.5,3.3) rectangle (7.5,4);
  \node at (6,3.65) {Write my own};
  \fill[gray!15] (0.5,1.8) rectangle (7.5,2.9);
  \node at (4,2.35) {Type a message...};
\end{tikzpicture}
...
\begin{tikzpicture}[
  ...
  \node[box, below=1.2cm of c, xshift=-2.5cm] (d) {使用建議開場白};
  \node[box, below=1.2cm of c, xshift=2.5cm] (e) {自己輸入訊息};
...
任務 2 YouTube & CAPTCHA 功能展示影片 & \textcolor{green!60!black}{完成} \\
任務 3 YouTube & 投票貼文功能展示影片 & \textcolor{green!60!black}{完成} \\
\bottomrule
\end{tabularx}
\end{table}
